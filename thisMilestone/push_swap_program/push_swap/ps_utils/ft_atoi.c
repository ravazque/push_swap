...
/*   Updated: 2024/12/16 12:54:01 by ravazque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	*ft_atoi(const char *nptr, int *res, int *index)
{
	...
	if (!parse_number(nptr, index, &num, sign))
		return (*res = 0, NULL);
	if ((nptr[*index] < '0' || nptr[*index] > '9') && nptr[*index] != '\0' && nptr[*index] != ' ')
		return (*res = 0, NULL);
	while (nptr[*index] == ' ')
		*index = *index + 1;
	if ((nptr[*index] < '0' || nptr[*index] > '9') && nptr[*index] != '\0' && nptr[*index] != '+' && nptr[*index] != '-')
		return (*res = 0, NULL);
	*res = num * sign;
	return (res);
}
