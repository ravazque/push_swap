...
/*   Updated: 2024/12/13 19:30:22 by ravazque         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_duplicates(t_stack *stack_a)
{
	...
	while (reference)
	{
		...
		while (temp)
		{
			...
			{
				write(STDERR_FILENO, "ErrorDuplicado\n", 15);
				return (1);
			}
			temp = temp->next;
		}
	...
	while (i < argc)
	{
		...
		while (argv[i][j] != '\0')
		{
			if (ft_atoi(argv[i], &value, &j) == NULL)
				return (write(STDERR_FILENO, "ErrorATOI_1\n", 12), 0);
			ft_insert_node(stack_a, value);
		}
		if (size == ft_lstsize(*stack_a))
			return (write(STDERR_FILENO, "ErrorATOI_2\n", 12), 0);
		i++;
	}
	return (1);
}
